...
class TasHardware(Node):
    ...
    def callback_laser_scan(self, LaserScan):
        object_position = self.get_object_position(LaserScan)
        current_time = self.get_clock.now()
        
        if self.last_position is not None and self.last_time is not None:
            change_position = object_position - self.last_object_position
            time_interval = (current_time - self.last_time).sec

            velocity = change_position / time_interval
            output_str = bytes('velocity'+ str(velocity) + '/n', 'ascii')
            self.ser.write(output_str)
