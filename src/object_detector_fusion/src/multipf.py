...
np.set_printoptions(precision=3) #set the ouputsprecision of numpy as 0.001
PRINT = False
SAVE_TO_FILE = False
...
class ParticleFilter(Node):
    ...
    def __init__(self):
        ...
        self.lidar_obstacle_sub = self.create_subscription(
            '/raw_obstacles/lidar_obstacles', Obstacles, self.lidar_callback,10)
        self.obstacle_sub = self.create_subscription(
            '/raw_obstacles/camera_obstacles', Obstacles, self.camera_callback,10)
        self.pf_sub = self.create_subscription(
            '/pub_pf', Float32, self.pf_callback,10)
        #self.obstacle_pub_mean = self.create_publisher(
            #'/estimate/viz/mean', Marker, queue_size=10)
        #self.obstacle_pub_particles = self.create_publisher(
            #'/estimate/viz/particles', Marker, queue_size=100)
        self.obstacle_pub_values = self.create_publisher(
            '/estimate/values/mean', Float32MultiArray, queue_size=10)
        self.pub_runtime = self.create_publisher(
    ...
    def initialize(self, initial_pos, iteration): #初始化粒子及其状态 
        initial_x = (initial_pos.center.x, initial_pos.center.y,
                     0.05, 0.05, 0, 0, initial_pos.true_radius) #定义了7个状态量 p v a 半径radius
        std = (0.2, 0.2, 0.5, 0.5, 0.5, 0.5, 0.1)
        self.new_particles[:, :, iteration] = pf.create_gaussian_particles(
            mean=initial_x, std=std, N=self.N) # 所有粒子 所有状态量 以及 某个迭代次数
        self.new_weights[:, iteration] = pf.create_initial_weights(self.N)

        self.old_camera_measurements.append(
            [initial_pos.center.x, initial_pos.center.y, initial_pos.true_radius]) # 将初始位置x,y和 半径储存在 old_camera_measurement这个列表中
        ...
        for particle in self.new_particles[:, :, iteration]:
            ...
            pub_particle.z = 0.35
            self.pub_particles.append(pub_particle) #储存着point类型的list

    def run_pf(self, camera, lidar, dt, i): #数据融合
    ...
    def cost_function(self, x_i, x_j, y_i, y_j, r_i, r_j): #匈牙利算法的cost function
        ...
        for i in range(len(cost_matrix)):
            for j in range(len(cost_matrix[i])):
                cost_matrix[i, j] = self.cost_function(
                    new[i, 0], old[j, 0], new[i, 1], old[j, 1], new[i, 2], old[j, 2])#计算平方差
        row_idx, col_idx = linear_sum_assignment(cost_matrix) #处理：linear_sum_assignment(cost_matrix) 调用对给定的成本矩阵执行算法，试图找到一种分配方式，使得总成本最小
    ...
    def lidar_callback(self, ros_data):
        ...
        for i in range(len(self.lidar_all)):
            ...
            new_lid_x = self.calibrate(lidar_circle.center.x, 1.0124, -0.0401) #这个校准值可调
    ...
    def pf_callback(self, ros_data):
        init_runtime = self.get_clock().now()
        ...
        if self.first_time:
            self.pf_is_running = [False for _ in range(self.pf_size)] #创建一个bool数组
        ...
        for i in range(len(self.camera_all)):
            ...
            self.new_camera_measurements.append(
                [new_cam_x, new_cam_y, camera_circle.true_radius]) #相机的消息 camera_circle中对 这里的true_radius进行更新
        ...
        for k in range(len(self.row_idx)):
            ...
            if camera_cost > 5:
                self.row_idx = np.delete(self.row_idx, k) #删除self.row_idx中的第k个元素
                #self.row_idx[k] = -1
                print("Blocked")
        ...
        if self.lidar_is_on:
            ...
            for k in range(len(self.lidar_row_idx)):
                ...
                if lidar_cost < 0.5: #对lidar数据进行匹配度计算  小于0.5 则进行数据融合
        ...
        for i in range(len(self.new_camera_measurements)):
            if not self.pf_is_running[i]:
                initial_pos = self.camera_all[i] #camera_all centerx centery 和 radius
                self.initialize(initial_pos, i) #粒子滤波器 ，不同的物体 对iteration
                self.run_pf(
            ...
            if PRINT:
                ...
                print('position estimate: ', self.mu[0:2])
    ...
    #     self.obstacle_pub_particles.publish(self.marker_particle)
        ...
        for i in range(len(self.row_idx)):
            self.old_camera_measurements[self.col_idx[i]] = self.new_camera_measurements[self.row_idx[i]]
