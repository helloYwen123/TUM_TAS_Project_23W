...
class ObjectTracker(Node):
    ...
    def callback_laser_scan(self,msg):
        ...
        if self.last_position is not None and self.last_time is not None:
            ...
            self.get_logger().info(f'the velocity of object is: {change_positions.shape} m/s')
            ...
            self.get_logger().info(f'the valid change_position is: {valid_change_positions} m/s')
            velocity = ...
            [(valid_change_position[0]/ time_interval, 
              valid_change_position[1]/ time_interval) for valid_change_position in valid_change_positions]
                       
            self.get_logger().info(f'the velocity of object is: {np.linalg.norm(velocity)} m/s')
            #publish
